...
def evalimage(net:Yolact, path:str, save_path:str=None):
    ...
    img_numpy,_ = prep_display(top_k = 4, dets_out=preds, img=frame,score_threshold = 0.5, class_color=False, undo_transform=False)
...
def evalvideo(net:Yolact, path:str, out_path:str=None):
    ...
    def prep_frame(inp, fps_str):
        with torch.no_grad():
            frame, preds = inp
            h,w=frame.shape[:2]
            return prep_display(top_k = 4, dets_out=preds, h=h, w=w, img=frame,score_threshold=0.5, undo_transform=False, class_color=True, fps_str=fps_str)
            #img_numpy, classes, scores, boxes, masks = prep_display(top_k = 4, dets_out=preds, h=h, w=w, img=frame,score_threshold=0.5, undo_transform=False, class_color=True, fps_str=fps_str)
            ...
            #
            #     h,w = mask.shape[:2]
            #     #print("mask shape : "+str((h,w)))
            #     filled_mask=np.zeros((w,h))
            #
            #
            ...
            #     filled_mask = cv2.morphologyEx(filled_mask, cv2.MORPH_CLOSE, kernel = (100,10))
            #
            #
            #     #print("bbox :" + str(bbox))
            #
    ...
    def segmentation( trained_model = None, config = None, cuda = False, img_file = None, img_path = None, detect=False, score_threshold=0.5,top_k=5, ap_data_file='results/ap_data.pkl',resume=False, display=True):
        ...
        with torch.no_grad():
            ...
            batch = FastBaseTransform()(frame.unsqueeze(0))
            preds = net(batch)
            classes, scores, boxes, masks = prep_display(top_k=4, dets_out=preds, img=frame, score_threshold=0.5, h=h, w=w, undo_transform=True, class_color=False, mask_alpha=0.45, fps_str='')
            print(boxes)
...
def evaluate(net:Yolact, dataset, train_mode=False):
    ...
    try:
        ...
        for it, image_idx in enumerate(dataset_indices):
            ...
            if args.display:
                img_numpy, _ = prep_display(dets_out=preds, img=img, h=h, w=w,score_threshold=0.5,top_k = 4)
            elif args.benchmark:

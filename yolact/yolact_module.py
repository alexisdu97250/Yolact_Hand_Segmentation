# -*- coding: utf-8 -*-
"""
Created on Thu May  7 15:31:32 2020
...
def prep_display(top_k, dets_out, img, score_threshold, h, w, undo_transform=True, class_color=False, mask_alpha=0.45, fps_str=''):
    """
    ...
    for j in range(num_dets_to_consider):
        if scores[j] < score_threshold:
            ...
            break
    return classes[:j+1], scores[:j+1], boxes[:j+1], masks
...
def create_sub_masks(mask_image):
    ##FIND ALL SUB-MASKS IN A SEGMENTATION IMAGE
...
def get_polygons(sub_mask):
    # Find contours (boundary lines) around each sub-mask
    # Note: there could be multiple contours if the object
    # is partially occluded. (E.g. an elephant behind a tree)

            contours = measure.find_contours(sub_mask, 0.7, positive_orientation='low')
            segmentations = []
            areas = []
            polygons = []
            ...
            if not multi_poly.is_empty:
                #print(len(multi_poly.geoms))
                for poly in multi_poly.geoms[1:]:
                    areas.append(poly.area)
                areas.sort()
                for poly in multi_poly.geoms[1:]:
                    #print("beginning new anno")
                    #print(poly)
                    area=poly.area
                    if(area in areas[-2:]): #Select the 2 polygons with the biggest area
                        segmentations.append(poly)
                        # x_array, y_array = poly.exterior.coords.xy
                        # x_array=list(x_array)
                        # y_array=list(y_array)
                        # segmentation = [] #Segmentation has the coords of points on the border of the polygon
                        # ## TO DO : FILL THE POLYGON
                        
                        
                        # for x in x_array:
                        #     for y in y_array:
                        #         segmentation.append((x,y))#Create a list of tuples
                        
                        # segmentations.append(segmentation)
            #print(segmentations)
            return polygons

def cv_contours(mask_image):
    #takes a grayscale mask image as input and returns all the contours found
    
    # Find Canny edges 
    kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE,(9,9))
    dilated = cv2.dilate(mask_image, kernel)
        
    #edged = cv2.Canny(255*dilated, 30, 200) 
    #cv2.waitKey(0) 
    ...
    # cv2.imshow('Contours',  255*mask_image) 
    # cv2.waitKey(0) 
    # cv2.destroyAllWindows()
    return contours
    ...
    def fill_mask(mask):
        ...
        for color,sub_mask in sub_masks.items():
            segmentations = get_polygons(sub_mask)
            w,h = mask.size
            for segmentation in segmentations:
                for x in range(w):
                    for y in range(h):
                        if point_inside_polygon(x, y, segmentation):
                            filled_mask[x][y]=255
                filled_mask = np.array(filled_mask)
                filled_masks.append(filled_mask)
                cv2.imshow('filled_mask',255*filled_mask)
                cv2.waitKey(0)
                cv2.destroyAllWindows()
            
            
def segmentation(img, trained_model, config, cuda, detect=False, score_threshold=0.5,top_k=5, ap_data_file='results/ap_data.pkl',resume=False, display=True):
    #Gives the detection bboxes. Bboxes is sorted by score so that bbox[0] is the bbox 
    #with the highest probability. bbox=[x1,y1,x2,y2] (TOP-LEFT, BOTTOM-RIGHT)
    ...
    with torch.no_grad():
        ...
        #Compute the bboxes and masks from image
        h, w, _= img.shape
        gray_img= cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
        frame = torch.from_numpy(img).cuda().float()
        batch = FastBaseTransform()(frame.unsqueeze(0))
        preds = net(batch)
        classes, scores, boxes, masks = prep_display(top_k, preds, frame, score_threshold, h, w, undo_transform=True, class_color=False, mask_alpha=0.45, fps_str='')
        ...
        #Use Shapely to find contours
        # print('mask converted to array \n')
        # print('mask shape ' + str(mask.shape))
        # print('image shape ' + str(img.shape))
        # polygons = get_polygons(Image.fromarray(mask))
        # print('number of polygons' + str(len(polygons)))
        # #Try to fill the polygon by detecting which points of the image are in the polygon
        ...
        #                 filled_mask[y][x]=255
        #     opening = cv2.morphologyEx(mask, cv2.MORPH_OPEN, kernel = (100,100))
        #     cv2.imshow('filled mask',opening)
        #     cv2.waitKey(0)
        #     cv2.destroyAllWindows()
        ...
        # cv2.waitKey(0)
        # cv2.destroyAllWindows()
        # #♣print("number of filled_masks : " + str(len(filled_masks)))
        
        #Try to fill the mask with morphological operations
        #closing = cv2.morphologyEx(mask, cv2.MORPH_CLOSE, kernel = (10,10)) 
        # cv2.imshow('mask',mask)
        # cv2.imshow('filled mask',closing)
        # cv2.waitKey(0)
        # cv2.destroyAllWindows()
        
        #Use OpenCV to find contours
        contours = cv_contours(np.uint8(mask))
        C = len(contours)

        cv2.drawContours(filled_mask, contours, -1, 255,thickness=-1) #Fills the biggest contour
...
def mirror(directory):
    listimg=glob(directory +'/*.jpg')
    for img_path in listimg:
        #flip the image and save it
        img = Image.open(img_path)
        if 'bis' not in img_path:
            img_pathbis = img_path.replace("frame_","framebis_")
            imgF = img.transpose(Image.FLIP_LEFT_RIGHT)
            imgF.save(img_pathbis)

def eval(directory, trained_model, config, cuda):
    listimg=glob(directory +'/*.jpg')
    for img_path in listimg:
        
        #Generate the masks and save them
        mask = 255*np.uint8(segmentation(img_path, trained_model=trained_model, config=config, cuda=cuda, detect=False, score_threshold=0.5,top_k=5, ap_data_file='results/ap_data.pkl',resume=False, display=True))
        rel_path= os.path.relpath(img_path,directory)
        cv2.imwrite(directory+'/masks/'+rel_path,mask)

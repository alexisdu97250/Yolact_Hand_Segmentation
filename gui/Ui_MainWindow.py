# -*- coding: utf-8 -*-
"""
Created on Mon Mar  8 16:58:40 2021
...
class Ui_MainWindow(object):
    ...
    def setupUi(self, MainWindow, fullPipeline):
        ...
        self.buttonA = QtWidgets.QPushButton(MainWindow)
        #self.labelA = QtWidgets.QLabel(self)
        self.buttonA.clicked.connect(self.clickCallback)
        self.buttonA.setText('Save')
        self.buttonA.move(792, 400)
        #self.labelA.setText(self)
        self.buttonA.move(792, 410)
        ...
        self.receiveBatch(np.zeros((20, 20, 3)), np.zeros((20, 20, 3)))
    ...
    def retranslateUi(self, MainWindow):
        ...
        #self.checkBox.setText(_translate("MainWindow", "Depth"))
    ...
    def handleComboBox(self):
        if self.comboBox.currentText() == "video file":
            ...
            #     _, video_2_path = self.getfiles("LEFT CAMERA FILE")
            #     cap_2 = cv2.VideoCapture(video_2_path)
            # else:
            #     cap_2 = cap_1

            fps_fixer_thread = FpsFixerThread(cap_1, self)
        ...
        if self.comboBox.currentText() == "image file":
            ...
            img_2, yolact_result = self.pipeline.process(img_1, img_2)
            if self.clickCallback:
                N = len([name for name in os.listdir("./test") if os.path.isfile(os.path.join("./test", name))])
                filename = "./test/frame_" + str(N) + ".jpg"
                cv2.imwrite(filename,yolact_result)
            self.receiveBatch(img_2, yolact_result)
        ...
        if self.comboBox.currentText() == "webcam":
            ...
            #cap_2 = cv2.VideoCapture(1) #UNCOMMENT FOR STEREOVISION
            cap_2=cap_1
    ...
    # def switchDepth(self, new_mode):
    #     self.pipeline.doDepth = new_mode

    # def switchSkeletton(self, new_mode):
    #     self.pipeline.doSkeleton = new_mode
    ...
    def receiveBatch(self, source, yolact):
